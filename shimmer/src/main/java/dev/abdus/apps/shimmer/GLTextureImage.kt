...
class GLTextureImage {
    ...
    fun load(imageSet: ImageSet) {
        ...
        if (textures.isNotEmpty() && loadedImageSetHash == newHash) return

        if (imageSet.original.isRecycled) return

        val bitmaps = listOf(imageSet.original) + imageSet.blurred
        val newAspectRatio = imageSet.width.toFloat() / imageSet.height
        val aspectRatioMatch = textures.isNotEmpty() && 
                               kotlin.math.abs(textureAspectRatio - newAspectRatio) < 0.001f
        
        val needsRealloc = textures.isEmpty() || 
                          !aspectRatioMatch ||
                          textureCount != bitmaps.size

        if (needsRealloc) {
            release()
            textures = IntArray(bitmaps.size)
            textureCount = bitmaps.size
            textureAspectRatio = newAspectRatio
            
            GLES30.glGenTextures(textures.size, textures, 0)

            for (i in textures.indices) {
                GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, textures[i])
                GLES30.glTexStorage2D(GLES30.GL_TEXTURE_2D, 1, GLES30.GL_RGBA8, 
        ...
        // Reuse existing textures - just upload new data
        for (i in bitmaps.indices) {
            GLES30.glBindTexture(GLES30.GL_TEXTURE_2D, textures[i])
            GLUtils.texSubImage2D(GLES30.GL_TEXTURE_2D, 0, 0, 0, bitmaps[i])
        }
    ...
    fun draw(
        ...
        GLES30.glUniform1i(handles.uniformTouchPointCount, touchIntensities.size)
        GLES30.glUniform3fv(handles.uniformTouchPoints, touchIntensities.size, touchPoints, 0)
        GLES30.glUniform1fv(handles.uniformTouchIntensities, touchIntensities.size, touchIntensities, 0)
